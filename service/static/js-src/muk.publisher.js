...
$.extend(muk, {
    publisher: {
        ...
        PublisherReportTemplate: function(params) {
            ...
            this.draw = function(edge) {
                this.edge = edge;
                var intro = "See the amount of APCs, total APC expenditure, or average APC cost for several publishers for a given period. Compare institutions and filter by journal type. Examples of how to use this report:                    <ul>                        <li>Compare APC number and expenditure with different publishers for similar institutions</li>                        <li>Compare average APC for different publishers</li>                    </ul>";
        ...
        Story: function(params) {
            ...
            this.draw = function() {
                ...
                story = story.replace(/{{x}}/g, Number(this.avgCount.toFixed(0)).toLocaleString()).replace(/{{y}}/g, Number(this.avgExp.toFixed(0)).toLocaleString()).replace(/{{z}}/g, Number(this.avgAPC.toFixed(0)).toLocaleString());
        ...
        averagesQuery: function(edge) {
            ...
            var aggSize = 10;
            var pubFilters = query.listMust(es.newTermsFilter({
            ...
            if (pubFilters.length === 0) {
                ...
                if (ibuckets.length > 0) {
                    ...
                    query.addMust(es.newTermsFilter({
                        field: "record.dcterms:publisher.name.exact",
                        values: terms
                    }))
                }
        ...
        globalAveragesQuery: function(edge) {
            ...
            query.removeMust(es.newTermFilter({
        ...
        reportDF: function(params) {
            ...
            var pubFilters = ch.edge.currentQuery.listMust(es.newTermsFilter({
                field: "record.dcterms:publisher.name.exact"
            }));
            var avgSeries = {};
            avgSeries["key"] = "UK Average";
            avgSeries["values"] = [];
            var avgPubFilters = ch.edge.realisedSecondaryQueries.avg.listMust(es.newTermsFilter({
                field: "record.dcterms:publisher.name.exact"
            }));
            ...
            var selected = ch.edge.getComponent({
                id: "institution"
            }).selected;
        ...
        apcCountDF: function(ch) {
            return muk.publisher.reportDF({
                chart: ch,
                valueFunction: function(bucket) {
                    return bucket.doc_count
                },
                avgFunction: function(bucket) {
                    return bucket.doc_count / bucket.institutions.value
                }
            })
        },
        apcExpenditureDF: function(ch) {
            return muk.publisher.reportDF({
                chart: ch,
                valueFunction: function(bucket) {
                    return bucket.publisher_stats.sum
                },
                avgFunction: function(bucket) {
                    return bucket.publisher_stats.sum / bucket.institutions.value
                }
            })
        },
        avgAPCDF: function(ch) {
            return muk.publisher.reportDF({
                chart: ch,
                valueFunction: function(bucket) {
                    return bucket.publisher_stats.avg
                },
                avgFunction: function(bucket) {
                    return bucket.publisher_stats.avg
                }
            ...
            var e = edges.newEdge({
                ...
                components: [edges.newMultiDateRangeEntry({
                    ...
                    renderer: edges.bs3.newNSeparateORTermSelectorRenderer({
                        ...
                        properties: [{
                            label: "Compare",
                            unselected: "<choose an institution>"
                        }, {
                            label: "with",
                            unselected: "<add another>"
                        }, {
                            label: "and",
                            unselected: "<add another>"
                        }],
                    ...
                    for(i in ts.values){
                        ...
                        if (val.count !==0 || !this.hideEmpty) {
                            if(($.inArray(val.term.toString(), ts.filters)) === -1 ) {
                                results += '<div class="form-fields__item-checkbox ' + valClass + '" data-key="' + edges.escapeHtml(val.term) + '"><label><input type="checkbox"/> ' + edges.escapeHtml(val.display) + '<span class="' + countClass + '"> (' + val.count + ")</span>" + "</label></div>"
                            }
                        }
                    }
                    results += '</div>';
                ...
                if (ts.filters.length > 0 && this.showSelected) {
                    ...
                    for (var i = 0; i < ts.filters.length; i++) {
                        var filt = ts.filters[i];
                            results += '<div class="form-fields__item-checkbox ' + filterRemoveClass + '" data-key="' + edges.escapeHtml(filt.term) + '"><label><input type="checkbox" checked/> ' + edges.escapeHtml(filt.display) + "</label></div>"
                    }
                    results += '</div>';
                }
                var tog = ts.display;
    ...
    bs3: {
        ...
        NSeparateORTermSelectorRenderer: function(params) {
            ...
            this.selectorChanged = function(element) {
                ...
                var triggered = this.component.selectTerms({
                    terms: vals,
                    clearOthers: true
                });
                if (!triggered) {
