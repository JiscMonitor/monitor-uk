...
$.extend(edges,{nvd3:{DataSeriesConversions:{toXY:function(data_series){var new_series=[];for(var i=0;i<data_series.length;i++){var os=data_series[i];var ns={};ns["key"]=os["key"];ns["values"]=[];for(var j=0;j<os.values.length;j++){var vector=os.values[j];ns["values"].push({x:vector.label,y:vector.value})}new_series.push(ns)}return new_series}},tools:{persistingPieColour:function(colours,persistence){if(!colours){colours=["#ea6ccb","#8fc8b0","#a9cf85","#d90d4c","#6c537e","#64d54f","#ecc7c4","#f1712b"]}if(!persistence){persistence={}}var i=0;return function(d,x){if(d.label in persistence){return persistence[d.label]}else{var c=colours[i%(colours.length-1)];i++;persistence[d.label]=c;return c}}}},newPieChartRenderer:function(params){if(!params){params={}}edges.nvd3.PieChartRenderer.prototype=edges.newRenderer(params);return new edges.nvd3.PieChartRenderer(params)},PieChartRenderer:function(params){this.showLabels=edges.getParam(params.showLabels,true);this.donut=edges.getParam(params.donut,false);this.labelThreshold=params.labelThreshold||.05;this.transitionDuration=params.transitionDuration||500;this.noDataMessage=params.noDataMessage||false;this.color=params.color||false;this.legendPosition=params.legendPosition||"top";this.labelsOutside=edges.getParam(params.labelsOutside,false);this.valueFormat=params.valueFormat||false;this.marginTop=params.marginTop||30;this.marginRight=params.marginRight||30;this.marginBottom=params.marginBottom||30;this.marginLeft=params.marginLeft||30;this.namespace="edges-nvd3-pie";this.draw=function(){var displayClasses=edges.css_classes(this.namespace,"display",this);var displayFrag="";if(this.component.display){displayFrag='<span class="'+displayClasses+'">'+this.component.display+"</span><br>"}var svgId=edges.css_id(this.namespace,"svg",this);var svgSelector=edges.css_id_selector(this.namespace,"svg",this);this.component.context.html(displayFrag+'<svg id="'+svgId+'"></svg>');var data_series=this.component.dataSeries;if(!data_series){data_series=[]}if(data_series.length>0){data_series=data_series[0].values}else{data_series=[]}var outer=this;nv.addGraph(function(){var chart=nv.models.pieChart().x(function(d){return d.label}).y(function(d){return d.value}).showLabels(outer.showLabels).legendPosition(outer.legendPosition).labelsOutside(outer.labelsOutside).margin({left:outer.marginLeft,right:outer.marginRight,top:outer.marginTop,bottom:outer.marginBottom});if(outer.noDataMessage){chart.noData(outer.noDataMessage)}if(outer.color){chart.color(outer.color)}if(outer.valueFormat){chart.valueFormat(outer.valueFormat)}d3.select(svgSelector).datum(data_series).transition().duration(outer.transitionDuration).call(chart);return chart})}},newHorizontalMultibarRenderer:function(params){if(!params){params={}}edges.nvd3.HorizontalMultibarRenderer.prototype=edges.newRenderer(params);return new edges.nvd3.HorizontalMultibarRenderer(params)},HorizontalMultibarRenderer:function(params){this.showValues=edges.getParam(params.showValues,true);this.toolTips=edges.getParam(params.toolTips,true);this.controls=edges.getParam(params.controls,false);this.stacked=edges.getParam(params.stacked,false);this.legend=edges.getParam(params.legend,true);this.color=params.color||false;this.noDataMessage=edges.getParam(params.noDataMessage,false);this.transitionDuration=params.transitionDuration||500;this.marginTop=params.marginTop||30;this.marginRight=params.marginRight||50;this.marginBottom=params.marginBottom||50;this.marginLeft=params.marginLeft||200;this.yTickFormat=edges.getParam(params.yTickFormat,",.0f");this.xTickFormat=edges.getParam(params.xTickFormat,false);this.valueFormat=edges.getParam(params.valueFormat,false);this.xAxisLabel=edges.getParam(params.xAxisLabel,false);this.yAxisLabel=edges.getParam(params.yAxisLabel,false);this.tooltipGenerator=edges.getParam(params.tooltipGenerator,false);this.namespace="edges-nvd3-horizontal-multibar";this.draw=function(){$(".nvtooltip").remove();var svgId=edges.css_id(this.namespace,"svg",this);var svgSelector=edges.css_id_selector(this.namespace,"svg",this);this.component.context.html('<svg id="'+svgId+'"></svg>');var data_series=this.component.dataSeries;if(!data_series){data_series=[]}var that=this;nv.addGraph(function(){var chart=nv.models.multiBarHorizontalChart().x(function(d){return d.label}).y(function(d){return d.value}).margin({top:that.marginTop,right:that.marginRight,bottom:that.marginBottom,left:that.marginLeft}).showValues(that.showValues).tooltips(that.toolTips).showControls(that.controls).showLegend(that.legend);if(that.stacked){chart.multibar.stacked(that.stacked)}if(that.yTickFormat){var fn=that.yTickFormat;if(typeof that.yTickFormat==="string"){fn=d3.format(that.yTickFormat)}chart.yAxis.tickFormat(fn)}if(that.yAxisLabel){chart.yAxis.axisLabel(that.yAxisLabel)}if(that.xTickFormat){var fn=that.xTickFormat;if(typeof that.xTickFormat==="string"){fn=d3.format(that.xTickFormat)}chart.xAxis.tickFormat(fn)}if(that.xAxisLabel){chart.xAxis.axisLabel(that.xAxisLabel)}if(that.valueFormat){var fn=that.valueFormat;if(typeof that.valueFormat==="string"){fn=d3.format(that.xTickFormat)}chart.valueFormat(fn)}if(that.noDataMessage){chart.noData(that.noDataMessage)}if(that.color){chart.color(that.color)}if(that.tooltipGenerator){chart.tooltip.contentGenerator(that.tooltipGenerator)}d3.select(svgSelector).datum(data_series).transition().duration(that.transitionDuration).call(chart);nv.utils.windowResize(chart.update);return chart})}},newMultibarRenderer:function(params){if(!params){params={}}edges.nvd3.MultibarRenderer.prototype=edges.newRenderer(params);return new edges.nvd3.MultibarRenderer(params)},MultibarRenderer:function(params){this.xTickFormat=params.xTickFormat||",.2f";this.yTickFormat=params.yTickFormat||",.2f";this.transitionDuration=params.transitionDuration||500;this.controls=edges.getParam(params.controls,false);this.barColor=params.barColor||false;this.showLegend=edges.getParam(params.showLegend,true);this.xAxisLabel=params.xAxisLabel||"";this.yAxisLabel=params.yAxisLabel||"";this.namespace="edges-nvd3-multibar";this.draw=function(){var displayClasses=edges.css_classes(this.namespace,"display",this);var displayFrag="";if(this.component.display){displayFrag='<span class="'+displayClasses+'">'+this.component.display+"</span><br>"}var svgId=edges.css_id(this.namespace,"svg",this);var svgSelector=edges.css_id_selector(this.namespace,"svg",this);this.component.context.html(displayFrag+'<svg id="'+svgId+'"></svg>');var data_series=this.component.dataSeries;if(!data_series){data_series=[]}data_series=edges.nvd3.DataSeriesConversions.toXY(this.component.dataSeries);var outer=this;nv.addGraph(function(){var chart=nv.models.multiBarChart().showControls(outer.controls);chart.xAxis.axisLabel(outer.xAxisLabel).tickFormat(d3.format(outer.xTickFormat));chart.yAxis.axisLabel(outer.yAxisLabel).tickFormat(d3.format(outer.yTickFormat));if(outer.barColor){chart.barColor(outer.barColor)}chart.showLegend(outer.showLegend);d3.select(svgSelector).datum(data_series).transition().duration(outer.transitionDuration).call(chart);nv.utils.windowResize(chart.update);return chart})}},newSimpleLineChartRenderer:function(params){if(!params){params={}}edges.nvd3.SimpleLineChartRenderer.prototype=edges.newRenderer(params);return new edges.nvd3.SimpleLineChartRenderer(params)},SimpleLineChartRenderer:function(params){this.interactiveGuideline=params.interactiveGuideline||true;this.xTickFormat=params.xTickFormat||",.2f";this.yTickFormat=params.yTickFormat||",.2f";this.transitionDuration=params.transitionDuration||500;this.lineColor=params.lineColor||false;this.includeOnY=params.includeOnY||false;this.showLegend=edges.getParam(params.showLegend,true);this.xAxisLabel=params.xAxisLabel||"";this.yAxisLabel=params.yAxisLabel||"";this.namespace="edges-nvd3-simple-line-chart";this.draw=function(){var displayClasses=edges.css_classes(this.namespace,"display",this);var displayFrag="";if(this.component.display){displayFrag='<span class="'+displayClasses+'">'+this.component.display+"</span><br>"}var svgId=edges.css_id(this.namespace,"svg",this);var svgSelector=edges.css_id_selector(this.namespace,"svg",this);this.component.context.html(displayFrag+'<svg id="'+svgId+'"></svg>');var data_series=this.component.dataSeries;if(!data_series){data_series=[]}var ds=edges.nvd3.DataSeriesConversions.toXY(data_series);var outer=this;nv.addGraph(function(){var chart=nv.models.lineChart().useInteractiveGuideline(outer.interactiveGuideline);chart.xAxis.axisLabel(outer.xAxisLabel).tickFormat(d3.format(outer.xTickFormat));if(outer.lineColor){chart.color(outer.lineColor)}if(outer.includeOnY){chart.forceY(outer.includeOnY)}chart.yAxis.axisLabel(outer.yAxisLabel).tickFormat(d3.format(outer.yTickFormat));chart.showLegend(outer.showLegend);d3.select(svgSelector).datum(ds).transition().duration(outer.transitionDuration).call(chart);nv.utils.windowResize(chart.update);return chart})}}}});
...
$.extend(edges,{newChart:function(params){if(!params){params={}}edges.Chart.prototype=edges.newComponent(params);return new edges.Chart(params)},Chart:function(params){this.category=params.category||"chart";this.display=params.display||"";this.dataSeries=params.dataSeries||false;this.dataFunction=params.dataFunction||false;this.dataFunctionClosure=params.dataFunctionClosure||false;this.aggregations=params.aggregations||[];this.dfArgs=params.dfArgs||{useAggregations:[],seriesKeys:{}};this.defaultRenderer=params.defaultRenderer||"newMultibarRenderer";this.init=function(edge){edges.newComponent().init.call(this,edge);for(var i=0;i<this.aggregations.length;i++){var agg=this.aggregations[i];if($.inArray(agg.name,this.dfArgs.useAggregations)===-1){this.dfArgs.useAggregations.push(agg.name)}}if(this.aggregations.length>0&&this.dataFunctionClosure){this.dataFunction=this.dataFunctionClosure(this.dfArgs)}};this.contrib=function(query){for(var i=0;i<this.aggregations.length;i++){query.addAggregation(this.aggregations[i])}};this.synchronise=function(){if(this.dataFunction){this.dataSeries=this.dataFunction(this)}}},ChartDataFunctions:{terms:function(params){var useAggregations=params.useAggregations||[];var seriesKeys=params.seriesKeys||{};return function(ch){var data_series=[];if(!ch.edge.result){return data_series}for(var i=0;i<useAggregations.length;i++){var agg=useAggregations[i];var buckets=ch.edge.result.data.aggregations[agg].buckets;var series={};series["key"]=seriesKeys[agg];series["values"]=[];for(var j=0;j<buckets.length;j++){var doccount=buckets[j].doc_count;var key=buckets[j].key;series.values.push({label:key,value:doccount})}data_series.push(series)}return data_series}},termsStats:function(params){var useAggregations=params.useAggregations||[];var seriesKeys=params.seriesKeys||{};var seriesFor=params.seriesFor||[];return function(ch){var data_series=[];if(!ch.edge.result){return data_series}for(var i=0;i<useAggregations.length;i++){var agg=useAggregations[i];var parts=agg.split(" ");for(var j=0;j<seriesFor.length;j++){var seriesStat=seriesFor[j];var series={};series["key"]=seriesKeys[agg+" "+seriesStat];series["values"]=[];var buckets=ch.edge.result.data.aggregations[parts[0]].buckets;for(var k=0;k<buckets.length;k++){var stats=buckets[k][parts[1]];var key=buckets[k].key;var val=stats[seriesStat];series.values.push({label:key,value:val})}data_series.push(series)}}return data_series}},recordsXY:function(params){var x=params.x;var x_default=params.x_default===undefined?0:params.x_default;var y=params.y;var y_default=params.y_default===undefined?0:params.y_default;var key=params.key;return function(ch){var data_series=[];if(!ch.edge.result){return data_series}var series={};series["key"]=key;series["values"]=[];var results=ch.edge.result.results();for(var i=0;i<results.length;i++){var res=results[i];var xval=edges.objVal(x,res,x_default);var yval=edges.objVal(y,res,y_default);series.values.push({label:xval,value:yval})}data_series.push(series);return data_series}},cumulativeXY:function(params){var x=params.x;var x_default=params.x_default===undefined?0:params.x_default;var y=params.y;var y_default=params.y_default===undefined?0:params.y_default;var key=params.key;var accumulate=params.accumulate||"y";return function(ch){var data_series=[];if(!ch.edge.result){return data_series}var series={};series["key"]=key;series["values"]=[];var total=0;var results=ch.edge.result.results();for(var i=0;i<results.length;i++){var res=results[i];var xval=edges.objVal(x,res,x_default);var yval=edges.objVal(y,res,y_default);if(accumulate==="x"){total+=xval;series.values.push({label:total,value:yval})}else if(accumulate==="y"){total+=yval;series.values.push({label:xval,value:total})}}data_series.push(series);return data_series}},totalledList:function(params){var listPath=params.listPath||"";var seriesKey=params.seriesKey||"";var keyField=params.keyField||false;var valueField=params.valueField||false;return function(ch){var data_series=[];if(!ch.edge.result){return data_series}var series={};series["key"]=seriesKey;series["values"]=[];var counter={};var results=ch.edge.result.results();for(var i=0;i<results.length;i++){var res=results[i];var l=edges.objVal(listPath,res,[]);for(var j=0;j<l.length;j++){var lo=l[j];var key=edges.objVal(keyField,lo,false);var value=edges.objVal(valueField,lo,0);if(key in counter){counter[key]+=value}else{counter[key]=value}}}for(key in counter){var val=counter[key];series.values.push({label:key,value:val})}data_series.push(series);return data_series}}},newPieChart:function(params){if(!params){params={}}edges.PieChart.prototype=edges.newChart(params);return new edges.PieChart(params)},PieChart:function(params){this.defaultRenderer=params.defaultRenderer||"newPieChartRenderer"},newHorizontalMultibar:function(params){if(!params){params={}}edges.HorizontalMultibar.prototype=edges.newChart(params);return new edges.HorizontalMultibar(params)},HorizontalMultibar:function(params){this.defaultRenderer=params.defaultRenderer||"newHorizontalMultibarRenderer"},newMultibar:function(params){if(!params){params={}}edges.Multibar.prototype=edges.newChart(params);return new edges.Multibar(params)},Multibar:function(params){this.defaultRenderer=params.defaultRenderer||"newMultibarRenderer"},newSimpleLineChart:function(params){if(!params){params={}}edges.SimpleLineChart.prototype=edges.newChart(params);return new edges.SimpleLineChart(params)},SimpleLineChart:function(params){this.xAxisLabel=params.xAxisLabel||"";this.yAxisLabel=params.yAxisLabel||"";this.defaultRenderer=params.defaultRenderer||"newSimpleLineChartRenderer"},newChartsTable:function(params){if(!params){params={}}edges.ChartsTable.prototype=edges.newComponent(params);return new edges.ChartsTable(params)},ChartsTable:function(params){this.chartComponents=edges.getParam(params.chartComponents,false);this.tabularise=edges.getParam(params.tabularise,false);this.defaultRenderer=params.defaultRenderer||"newChartsTableRenderer";this.results=[];this.synchronise=function(){this.results=[];if(!this.chartComponents){return}var comps=[];for(var i=0;i<this.edge.components.length;i++){var comp=this.edge.components[i];if($.inArray(comp.id,this.chartComponents)>-1){comps.push(comp)}}if(this.tabularise){this.results=this.tabularise(comps)}}}});
...
$.extend(true,edges,{bs3:{newRefiningANDTermSelectorRenderer:function(params){if(!params){params={}}edges.bs3.RefiningANDTermSelectorRenderer.prototype=edges.newRenderer(params);return new edges.bs3.RefiningANDTermSelectorRenderer(params)},RefiningANDTermSelectorRenderer:function(params){this.hideInactive=edges.getParam(params.hideInactive,false);this.controls=edges.getParam(params.controls,true);this.open=edges.getParam(params.open,false);this.togglable=edges.getParam(params.togglable,true);this.showSelected=edges.getParam(params.showSelected,true);this.sortCycle=edges.getParam(params.sortCycle,["count desc","count asc","term desc","term asc"]);this.namespace="edges-bs3-refiningand-term-selector";this.draw=function(){var ts=this.component;var namespace=this.namespace;var resultsListClass=edges.css_classes(namespace,"results-list",this);var resultClass=edges.css_classes(namespace,"result",this);var valClass=edges.css_classes(namespace,"value",this);var controlClass=edges.css_classes(namespace,"controls",this);var filterRemoveClass=edges.css_classes(namespace,"filter-remove",this);var facetClass=edges.css_classes(namespace,"facet",this);var headerClass=edges.css_classes(namespace,"header",this);var selectedClass=edges.css_classes(namespace,"selected",this);var controlId=edges.css_id(namespace,"controls",this);var sizeId=edges.css_id(namespace,"size",this);var orderId=edges.css_id(namespace,"order",this);var toggleId=edges.css_id(namespace,"toggle",this);var resultsId=edges.css_id(namespace,"results",this);var results="Loading...";if(ts.values!==false){results="No data available"}if(ts.values&&ts.values.length>0){results="";var filterTerms=[];for(var i=0;i<ts.filters.length;i++){filterTerms.push(ts.filters[i].term.toString())}for(var i=0;i<ts.values.length;i++){var val=ts.values[i];if($.inArray(val.term.toString(),filterTerms)===-1){results+='<div class="'+resultClass+'"><a href="#" class="'+valClass+'" data-key="'+edges.escapeHtml(val.term)+'">'+edges.escapeHtml(val.display)+"</a> ("+val.count+")</div>"}}}var controlFrag="";if(this.controls){var ordering='<a href="#" title=""><i class="glyphicon glyphicon-arrow-up"></i></a>';controlFrag='<div class="'+controlClass+'" style="display:none" id="'+controlId+'"><div class="row">                         <div class="col-md-12">                            <div class="btn-group">                                <button type="button" class="btn btn-default btn-sm" id="'+sizeId+'" title="List Size" href="#">0</button>                                 <button type="button" class="btn btn-default btn-sm" id="'+orderId+'" title="List Order" href="#"></button>                             </div>                        </div>                    </div></div>'}var filterFrag="";if(ts.filters.length>0&&this.showSelected){for(var i=0;i<ts.filters.length;i++){var filt=ts.filters[i];filterFrag+='<div class="'+resultClass+'"><strong>'+edges.escapeHtml(filt.display)+"&nbsp;";filterFrag+='<a href="#" class="'+filterRemoveClass+'" data-key="'+edges.escapeHtml(filt.term)+'">';filterFrag+='<i class="glyphicon glyphicon-black glyphicon-remove"></i></a>';filterFrag+="</strong></a></div>"}}var tog=ts.display;if(this.togglable){tog='<a href="#" id="'+toggleId+'"><i class="glyphicon glyphicon-plus"></i>&nbsp;'+tog+"</a>"}var frag='<div class="'+facetClass+'">                        <div class="'+headerClass+'"><div class="row">                             <div class="col-md-12">                                '+tog+'                            </div>                        </div></div>                        {{CONTROLS}}                        <div class="row" style="display:none" id="'+resultsId+'">                            <div class="col-md-12">                                <div class="'+selectedClass+'">{{SELECTED}}</div>                                <div class="'+resultsListClass+'">{{RESULTS}}</div>                            </div>                        </div></div>';frag=frag.replace(/{{RESULTS}}/g,results).replace(/{{CONTROLS}}/g,controlFrag).replace(/{{SELECTED}}/g,filterFrag);ts.context.html(frag);this.setUISize();this.setUISort();this.setUIOpen();var valueSelector=edges.css_class_selector(namespace,"value",this);var filterRemoveSelector=edges.css_class_selector(namespace,"filter-remove",this);var toggleSelector=edges.css_id_selector(namespace,"toggle",this);var sizeSelector=edges.css_id_selector(namespace,"size",this);var orderSelector=edges.css_id_selector(namespace,"order",this);edges.on(valueSelector,"click",this,"termSelected");edges.on(toggleSelector,"click",this,"toggleOpen");edges.on(filterRemoveSelector,"click",this,"removeFilter");edges.on(sizeSelector,"click",this,"changeSize");edges.on(orderSelector,"click",this,"changeSort")};this.setUIOpen=function(){var resultsSelector=edges.css_id_selector(this.namespace,"results",this);var controlsSelector=edges.css_id_selector(this.namespace,"controls",this);var toggleSelector=edges.css_id_selector(this.namespace,"toggle",this);var results=this.component.jq(resultsSelector);var controls=this.component.jq(controlsSelector);var toggle=this.component.jq(toggleSelector);if(this.open){toggle.find("i").removeClass("glyphicon-plus").addClass("glyphicon-minus");controls.show();results.show()}else{toggle.find("i").removeClass("glyphicon-minus").addClass("glyphicon-plus");controls.hide();results.hide()}};this.setUISize=function(){var sizeSelector=edges.css_id_selector(this.namespace,"size",this);this.component.jq(sizeSelector).html(this.component.size)};this.setUISort=function(){var orderSelector=edges.css_id_selector(this.namespace,"order",this);var el=this.component.jq(orderSelector);if(this.component.orderBy==="count"){if(this.component.orderDir==="asc"){el.html('count <i class="glyphicon glyphicon-arrow-down"></i>')}else if(this.component.orderDir==="desc"){el.html('count <i class="glyphicon glyphicon-arrow-up"></i>')}}else if(this.component.orderBy==="term"){if(this.component.orderDir==="asc"){el.html('a-z <i class="glyphicon glyphicon-arrow-down"></i>')}else if(this.component.orderDir==="desc"){el.html('a-z <i class="glyphicon glyphicon-arrow-up"></i>')}}};this.termSelected=function(element){var term=this.component.jq(element).attr("data-key");this.component.selectTerm(term)};this.removeFilter=function(element){var term=this.component.jq(element).attr("data-key");this.component.removeFilter(term)};this.toggleOpen=function(element){this.open=!this.open;this.setUIOpen()};this.changeSize=function(element){var newSize=prompt("Currently displaying "+this.component.size+" results per page. How many would you like instead?");if(newSize){this.component.changeSize(parseInt(newSize))}};this.changeSort=function(element){var current=this.component.orderBy+" "+this.component.orderDir;var idx=$.inArray(current,this.sortCycle);var next=this.sortCycle[(idx+1)%4];var bits=next.split(" ");this.component.changeSort(bits[0],bits[1])}}}});
...
$.extend(true,edges,{bs3:{newTabularResultsRenderer:function(params){if(!params){params={}}edges.bs3.TabularResultsRenderer.prototype=edges.newRenderer(params);return new edges.bs3.TabularResultsRenderer(params)},TabularResultsRenderer:function(params){this.noResultsText=params.noResultsText||"No results to display";this.defaultCellContent=params.defaultCellContent||"-";this.fieldDisplay=params.fieldDisplay||[];this.displayListedOnly=edges.getParam(params.displayListedOnly,true);this.headerOrderingFunction=edges.getParam(params.headerOrderingFunction,false);this.download=edges.getParam(params.download,false);this.downloadText=edges.getParam(params.downloadText,"download");this.downloadPrefix=edges.getParam(params.downloadPrefix,"download");this.namespace="edges-bs3-tabular-results";this.draw=function(){var frag=this.noResultsText;if(this.component.results===false){frag=""}var results=this.component.results;if(results&&results.length>0){var headerKeys=this._getHeaderRow();var tableClasses=edges.css_classes(this.namespace,"table",this);var headerClasses=edges.css_classes(this.namespace,"header",this);var cellClasses=edges.css_classes(this.namespace,"cell",this);var downloadClasses=edges.css_classes(this.namespace,"download",this);var tableDivClasses=edges.css_classes(this.namespace,"tablediv",this);var downloadId=edges.css_id(this.namespace,"download",this);var down="";if(this.download){down='<div class="row"><div class="col-md-12"><div class="'+downloadClasses+'"><a href="#" id="'+downloadId+'">'+edges.escapeHtml(this.downloadText)+"</a></div></div></div>"}var frag='<div class="table-responsive '+tableDivClasses+'">';frag+='<table class="'+tableClasses+'"><thead><tr>_HEADERS_</tr></thead><tbody>';var headerDisplay=[];for(var i=0;i<headerKeys.length;i++){var trip=false;for(var j=0;j<this.fieldDisplay.length;j++){var fd=this._getFieldDisplay(headerKeys[i]);if(fd){headerDisplay.push(fd.display);trip=true;break}}if(!trip){headerDisplay.push(headerKeys[i])}}var headers="";for(var i=0;i<headerDisplay.length;i++){var header=headerDisplay[i];headers+='<th class="'+headerClasses+'">'+header+"</th>"}frag=frag.replace(/_HEADERS_/g,headers);for(var i=0;i<results.length;i++){var res=results[i];frag+="<tr>";for(var j=0;j<headerKeys.length;j++){var key=headerKeys[j];var def=this.defaultCellContent;var fd=this._getFieldDisplay(key);if(fd){def=fd.default}var val=edges.objVal(key,res,def);var fieldClasses=edges.css_classes(this.namespace,"cell-"+edges.safeId(key),this);frag+='<td class="'+cellClasses+" "+fieldClasses+'">'+edges.escapeHtml(val)+"</td>"}frag+="</tr>"}frag+="</tbody></table></div>"+down}this.component.context.html(frag);if(this.download){var downloadIdSelector=edges.css_id_selector(this.namespace,"download",this);edges.on(downloadIdSelector,"click",this,"doDownload")}};this.doDownload=function(element){if(!this.download){return}var downloadInfo=this._downloadData();var blob=new Blob([downloadInfo.data],{type:downloadInfo.fileType});var url=window.URL.createObjectURL(blob);var a=document.createElement("a");document.body.appendChild(a);a.style="display: none";a.href=url;a.download=downloadInfo.fileName;a.click();window.URL.revokeObjectURL(url)};this._downloadData=function(){if(!this.download){return false}var table=[];var results=this.component.results;if(results&&results.length>0){var headerKeys=this._getHeaderRow();var headerDisplay=[];for(var i=0;i<headerKeys.length;i++){var trip=false;for(var j=0;j<this.fieldDisplay.length;j++){var fd=this._getFieldDisplay(headerKeys[i]);if(fd){headerDisplay.push(fd.display);trip=true;break}}if(!trip){headerDisplay.push(headerKeys[i])}}table.push(headerDisplay);for(var i=0;i<results.length;i++){var res=results[i];var row=[];for(var j=0;j<headerKeys.length;j++){var key=headerKeys[j];var val=edges.objVal(key,res,"");row.push(val)}table.push(row)}}var data=edges.csv.serialise({data:table});var fileType="text/csv;charset=UTF-8";var extension="csv";var stamp=(new Date).getTime();var fileName=this.downloadPrefix+"_"+stamp+"."+extension;return{data:data,fileType:fileType,fileName:fileName}};this._getFieldDisplay=function(field){for(var j=0;j<this.fieldDisplay.length;j++){if(this.fieldDisplay[j].field==field){return this.fieldDisplay[j]}}return false};this._getHeaderRow=function(){if(this.headerOrderingFunction){return this.headerOrderingFunction(this)}var results=this.component.results;var hasResults=results!==false&&results.length>0;if(!hasResults||this.fieldDisplay.length==0&&this.displayListedOnly){return[]}var headers=[];if(this.fieldDisplay.length>0){for(var i=0;i<this.fieldDisplay.length;i++){headers.push(this.fieldDisplay[i].field)}}if(!this.displayListedOnly){var keySet={};for(var i=0;i<results.length;i++){var ks=Object.keys(results[i]);for(var j=0;j<ks.length;j++){if(!(ks[j]in keySet)){keySet[ks[j]]=true}}}var keys=Object.keys(keySet);keys.sort();for(var i=0;i<keys.length;i++){if($.inArray(keys[i],headers)==-1){headers.push(keys[i])}}}return headers}}}});
$.extend(true,edges,{bs3:{newBSMultiDateRange:function(params){if(!params){params={}}edges.bs3.BSMultiDateRange.prototype=edges.newRenderer(params);return new edges.bs3.BSMultiDateRange(params)},BSMultiDateRange:function(params){this.dateFormat=edges.getParam(params.dateFormat,"MMMM D, YYYY");this.useSelect2=edges.getParam(params.useSelect2,false);this.ranges=edges.getParam(params.ranges,false);this.dre=false;this.selectId=false;this.rangeId=false;this.selectJq=false;this.rangeJq=false;this.drp=false;this.namespace="edges-bs3-bs-multi-date-range";this.draw=function(){var dre=this.component;var selectClass=edges.css_classes(this.namespace,"select",this);var inputClass=edges.css_classes(this.namespace,"input",this);var prefixClass=edges.css_classes(this.namespace,"prefix",this);this.selectId=edges.css_id(this.namespace,dre.id+"_date-type",this);this.rangeId=edges.css_id(this.namespace,dre.id+"_range",this);var pluginId=edges.css_id(this.namespace,dre.id+"_plugin",this);var options="";for(var i=0;i<dre.fields.length;i++){var field=dre.fields[i];var selected=dre.currentField==field.field?' selected="selected" ':"";options+='<option value="'+field.field+'"'+selected+">"+field.display+"</option>"}var frag='<div class="form-inline">';if(dre.display){frag+='<span class="'+prefixClass+'">'+dre.display+"</span>"}frag+='<div class="form-group"><select class="'+selectClass+' form-control" name="'+this.selectId+'" id="'+this.selectId+'">'+options+"</select></div>";frag+='<div id="'+this.rangeId+'" class="'+inputClass+' form-control">                    <i class="glyphicon glyphicon-calendar"></i>&nbsp;                    <span></span> <b class="caret"></b>                </div>';frag+="</div>";dre.context.html(frag);var selectIdSelector=edges.css_id_selector(this.namespace,dre.id+"_date-type",this);var rangeIdSelector=edges.css_id_selector(this.namespace,dre.id+"_range",this);this.selectJq=dre.jq(selectIdSelector);this.rangeJq=dre.jq(rangeIdSelector);var cb=edges.objClosure(this,"updateDateRange",["start","end"]);var props={locale:{format:"DD/MM/YYYY"},opens:"left"};if(this.ranges){props["ranges"]=this.ranges}var pluginSelector=edges.css_id_selector(this.namespace,dre.id+"_plugin",this);$(pluginSelector).remove();this.rangeJq.daterangepicker(props,cb);this.drp=this.rangeJq.data("daterangepicker");this.drp.container.attr("id",pluginId).addClass("show-calendar");this.prepDates();if(this.useSelect2){this.selectJq.select2()}edges.on(selectIdSelector,"change",this,"typeChanged")};this.dateRangeDisplay=function(params){var start=params.start;var end=params.end;this.rangeJq.find("span").html(start.utc().format(this.dateFormat)+" - "+end.utc().format(this.dateFormat))};this.updateDateRange=function(params){var start=params.start;var end=params.end;var date_type=null;if(this.useSelect2){date_type=this.selectJq.select2("val")}else{date_type=this.selectJq.val()}this.component.changeField(date_type);this.component.setFrom(start.toDate());this.component.setTo(end.toDate());this.dateRangeDisplay(params);var triggered=this.component.triggerSearch();if(!triggered){this.prepDates()}};this.typeChanged=function(element){var date_type=null;if(this.useSelect2){date_type=this.selectJq.select2("val")}else{date_type=this.selectJq.val()}this.component.changeField(date_type);this.component.setFrom(false);this.component.setTo(false);var triggered=this.component.triggerSearch();if(!triggered){this.prepDates()}};this.prepDates=function(){var min=this.component.currentEarliest();var max=this.component.currentLatest();var fr=this.component.fromDate;var to=this.component.toDate;if(min){this.drp.minDate=moment(min);this.drp.setStartDate(moment(min))}else{this.drp.minDate=moment(this.component.defaultEarliest);this.drp.setStartDate(moment(this.component.defaultEarliest))}if(max){this.drp.maxDate=moment(max);this.drp.setEndDate(moment(max))}else{this.drp.maxDate=moment(this.component.defaultLatest);this.drp.setEndDate(moment(this.component.defaultLatest))}if(fr){if(fr<this.drp.minDate){this.drp.minDate=moment(fr)}if(fr>this.drp.maxDate){this.drp.maxDate=moment(fr)}this.drp.setStartDate(moment(fr))}if(to){if(to<this.drp.minDate){this.drp.minDate=moment(to)}if(to>this.drp.maxDate){this.drp.maxDate=moment(to)}this.drp.setEndDate(moment(to))}this.dateRangeDisplay({start:this.drp.startDate,end:this.drp.endDate})}}}});
